...
from file_names_for_data import S3_CLIENT 
from file_names_for_data import S3_BUCKET_NAME
from file_names_for_data import S3_FILE_SUFFIX
from algo_feature_engineering.data_load_utils import (
    load_ticker_list,
    download_ticker_data
)
from my_functions import calculate_daily_trend_consolidated
from my_functions import calculate_daily_ema
from my_functions import calculate_daily_hma
from my_functions import calculate_daily_rsi
from my_functions import add_initial_price_increase_z_score
from my_functions import calculate_daily_slope
from my_functions import add_initial_price_increase
from my_functions import calculate_daily_percentage_above
from my_functions import calculate_daily_cumulative_min
...
# CONFIGURATION
######################
USE_SELECTED_SYMBOLS = True  # Set to False to process all symbols

# Defined set of symbols to process when USE_SELECTED_SYMBOLS is True

SELECTED_SYMBOLS = ["TQQQ"]
#SELECTED_SYMBOLS = ["BTCUSD", "ETHUSD", "SOLUSD", "ADAUSD", "DOGEUSD", "XRPUSD", "AVAXUSD", "TRXUSD", "TONUSD", "XLMUSD", "SHIBUSD", "DOTUSD", "LINKUSD", "BCHUSD", "SUIUSD"]
#SELECTED_SYMBOLS = unique_combined_symbols
...
unique_combined_symbol_prefix_list = list(unique_combined_symbol_prefix_list)

if USE_SELECTED_SYMBOLS:
    # Filter to include only selected symbols
    top_symbols = [
        (symbol, s3_prefix) 
        for symbol, s3_prefix in unique_combined_symbol_prefix_list 
        if symbol in SELECTED_SYMBOLS
    ]
else:
    # Use all symbols
    top_symbols = unique_combined_symbol_prefix_list
...
for symbol, s3_prefix in tqdm(top_symbols, desc="Processing symbols"):
    ...
    # Create reference to the dataframe for cleaner code
